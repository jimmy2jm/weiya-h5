    ...
    return new Promise(function(resolve) {
        ...
        var updateVideoProgress = function() {
            ...
            if (video.buffered.length > 0 && isFinite(video.duration) && video.duration > 0) {
                ...
                updateProgress(Math.min(progress1, 99));
            }
        };
        ...
        elements.video1.addEventListener('loadedmetadata', function() {
            ...
            updateVideoProgress();
            // iOS微信：元数据加载后进度至少30%
            if (progress1 < 30) updateProgress(30);
        });
        ...
        elements.video1.addEventListener('loadeddata', function() {
            updateVideoProgress();
            if (progress1 < 50) updateProgress(50);
            // iOS微信关键：loadeddata 就可以认为准备好了
            log('视频1 loadeddata 触发，readyState: ' + elements.video1.readyState);
        });
        
        // canplay - iOS微信中这个事件更可靠
        elements.video1.addEventListener('canplay', function() {
            updateVideoProgress();
            if (progress1 < 80) updateProgress(80);
            log('视频1 canplay 触发');
            // iOS微信：canplay就可以开始了
            completeLoading('canplay');
        });
        
        // canplaythrough - 最理想情况
        elements.video1.addEventListener('canplaythrough', function() {
            completeLoading('canplaythrough');
        }, { once: true });
        ...
        // iOS微信专用：更短的超时，使用更宽松的条件
        setTimeout(function() {
            if (!video1Ready) {
                var readyState = elements.video1.readyState;
                log('3秒检查: readyState=' + readyState);
                // readyState >= 2 (HAVE_CURRENT_DATA) 就可以了
                if (readyState >= 2) {
                    completeLoading('timeout-ready');
                }
            }
        }, 3000);
